...
def create_request(method="GET", query_params=None, headers=None, body=b""):
    ...
    scope = {
        ...
        "query_string": (
            b""
            if not query_params
            else "&".join([f"{k}={v}" for k, v in query_params.items()]).encode()
        ),
...
async def test_webhook_handle_success(webhook_handlers, tool_env):
    ...
    req = create_request(method="POST", headers={"X-Hub-Signature-256": f"sha256={sig}"}, body=body)
...
async def test_webhook_handle_invalid_sig(webhook_handlers):
    handle = webhook_handlers["meta_webhook_handler"]
    req = create_request(
        method="POST", headers={"X-Hub-Signature-256": "sha256=invalid"}, body=b"{}"
    )
    resp = await handle(req)
    assert resp.status_code == 403
