...
async def test_all_wrappers(monkeypatch):
    ...
    await sdk.insights_ads_account(
        InsightsAdsAccount(
            ad_account_id="a",
            fields=["f"],
            level="campaign",
    ...
    await sdk.assets_video_upload_init(
        AssetsVideoUploadInit(page_id="p", file_size=1, file_name="f")
    )
    await sdk.assets_video_upload_chunk(
        AssetsVideoUploadChunk(upload_session_id="s", start_offset=0, chunk=b"")
    )
    await sdk.assets_video_upload_finish(AssetsVideoUploadFinish(upload_session_id="s"))
    await sdk.assets_video_subtitles_upload(
        AssetsVideoSubtitlesUpload(video_id="v", lang="en", srt_buffer=b"")
    )
    ...
    # Ads
    await sdk.ads_campaigns_create(
        AdsCampaignCreate(ad_account_id="a", name="n", objective="o", status="s")
    )
    ...
    await sdk.ads_calendar_note_put(
        AdsCalendarNotePut(idempotency_key="k", subject="s", when=datetime.now(), related_ids=[])
    )

    # Helpers
    await sdk.ad_library_search(
        AdLibrarySearch(ad_type="a", ad_reached_countries=["US"], fields=["f"])
    )
    await sdk.ad_library_search_by_pages(
        AdLibraryByPage(page_ids=["p"], ad_type="a", ad_reached_countries=["US"], fields=["f"])
    )
